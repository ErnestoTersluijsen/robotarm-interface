...
void RosInterface::handle_emergency_stop_request([[maybe_unused]] const std::shared_ptr<robotarm_hld::srv::EmergencyStop::Request> request, std::shared_ptr<robotarm_hld::srv::EmergencyStop::Response> response)
{
	...
	print_state_info("Emergency stop");
}

void RosInterface::print_state_info(std::string state)
{
	RCLCPP_INFO(this->get_logger(), "STATE:{ %s }", state.c_str());
}

void RosInterface::print_event_info(std::string event)
{
	RCLCPP_DEBUG(this->get_logger(), "EVENT:{ %s }", event.c_str());
}